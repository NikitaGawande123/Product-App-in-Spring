<html>
<head>
<%@include file="./base.jsp"%>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
</head>
<body>
	<div class="container mt-3">

		<div class="row">

			<div class="col-md-12">

				<h1 class="text-center mb-3">Welcome to Product App</h1>
				
	

<div class="row">
    <form method="get" action="/search/product-search">
       <div class="small-3 columns">
          <input type="text" id ="search-input" class="forn-control" placeholder="enter product name" >
       </div>

       <div class="search-result">
          <button id="button-id" type="submit">Search Product</button>
       </div>

       <div>
          <a href="search">search</a>
       </div>
    </form>

</div>
				
				</div>
				
				</div>

				<table class="table">
					<thead class="thead-dark">
						<tr>
							<th scope="col">ID</th>
							<th scope="col">Product Name</th>
							<th scope="col">Description</th>
							<th scope="col">Price</th>
							<th scope="col">Action</th>
							<th scope="col">Time</th>
						</tr>
					</thead>
					<tbody>

						<c:forEach items="${products }" var="p">
							<tr>
								<th scope="row">TECHONLY${p.id }</th>
								<td>${p.name }</td>
								<td>${p.description }</td>
								<td class="font-weight-bold">&#x20B9; ${p.price }</td>
								<td><a href="delete/${p.id }"><i
										class="fas fa-trash text-danger" style="font-size: 30px;"></i></a>
									<a href="update/${p.id }"><i
										class="fas fa-pen-nib text-primary" style="font-size: 30px;"></i></a>
										<td width="30%"><b>&nbsp;UT - :&nbsp; <font color="#FF0000">


<%= new java.util.Date() %>
</font></b></td>
                                   </td>
								
							</tr>
						</c:forEach>
					</tbody>
				</table>

				<div class="container text-center">

					<a href="add-product" class="btn btn-outline-success">Add
						Product</a>

				</div>


			</div>


		</div>

	</div>
</body>
</html>
